...
import { LogOut, LayoutDashboard, Heart, FileText, Mail, Settings, Loader2, Send, Plus, X, Bold, Italic, Link2, Image, AlignLeft, AlignCenter, Type, Palette, Users, Search, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function MessageriePage() {
  ...
  const openButtonModal = () => {
    if (editorRef.current) {
      ...
      if (!url) return
      const text = window.prompt('Texte du bouton :', 'Reserver mes places')
      if (!text) return
  ...
  const addAllFiltered = () => {
    ...
    const newEmails = filtered.map(c => c.email).filter(e => !recipients.includes(e))
    ...
    <div className="min-h-screen flex" style={{ background: '#f5f3ef' }}>
      <aside className="hidden md:flex flex-col w-[240px] border-r" style={{ background: '#fff', borderColor: '#e5e2dc' }}>
        ...
        <nav className="flex-1 px-3 py-4">
          {nav.map((item) => {
            const active = item.href === '/dashboard/messagerie'
            return (
      ...
      <main className="flex-1 overflow-auto">
        <div className="p-6 md:p-10 max-w-4xl">
          <div className="mb-8">
            <h2 style={{ fontSize: '24px', fontWeight: 700, color: '#1a1a1a' }}>Nouvelle newsletter</h2>
          ...
          <div className="mb-5">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setShowContacts(!showContacts)}
                style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', color: '#722f37', background: 'none', border: '1px solid #722f37', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer' }}>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => addRecipient()}
                style={{ background: '#722f37', color: '#fff', border: 'none', borderRadius: '8px', padding: '0 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '13px', whiteSpace: 'nowrap' }}>
            ...
            {/* Contact picker */}
            {showContacts && (
              <div className="mt-3 rounded-xl p-4" style={{ background: '#fff', border: '1px solid #e5e2dc' }}>
                <div className="flex gap-2 mb-3">
                  ...
                  <button onClick={addAllFiltered}
                    style={{ fontSize: '12px', padding: '0 12px', background: '#722f37', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                    + Ajouter tous ({filteredContacts.length})
                  </button>
                </div>
                <div style={{ maxHeight: '200px', overflow: 'auto' }}>
                  {filteredContacts.slice(0, 50).map(c => {
                    ...
                    return (
                      <div key={c._id} className="flex items-center justify-between py-1.5 px-2 rounded hover:bg-gray-50" style={{ fontSize: '13px' }}>
                        <div>
                          <span style={{ fontWeight: 500, color: '#1a1a1a' }}>{c.firstName} {c.lastName}</span>
                          <span style={{ color: '#999', marginLeft: '8px' }}>{c.email}</span>
                        </div>
                        {added ? (
                          <span style={{ fontSize: '11px', color: '#16a34a' }}>Ajoute</span>
                        ) : (
                          <button onClick={() => addRecipient(c.email)}
                            style={{ fontSize: '11px', color: '#722f37', background: 'none', border: '1px solid #e5e2dc', borderRadius: '4px', padding: '2px 8px', cursor: 'pointer' }}>
                            + Ajouter
                          </button>
                        )}
                      </div>
            ...
            {recipients.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {recipients.slice(0, 20).map((r) => (
                  <span key={r} className="flex items-center gap-1 rounded-full px-3 py-1 text-sm" style={{ background: '#f0eee9', color: '#555' }}>
                    {r}
          ...
          <div className="mb-5">
            ...
            {!showSource && (
              <div className="flex flex-wrap gap-1 mb-0 p-2 rounded-t-lg" style={{ background: '#fff', border: '1px solid #ddd', borderBottom: 'none' }}>
                ...
                <div style={{ width: '1px', background: '#e5e2dc', margin: '0 4px' }} />
            ...
            {showSource ? (
              <textarea value={htmlSource} onChange={(e) => setHtmlSource(e.target.value)}
                style={{ width: '100%', height: '400px', padding: '14px', fontSize: '13px', fontFamily: 'monospace', color: '#1a1a1a', background: '#f7f6f3', border: '1px solid #ddd', borderRadius: '8px', outline: 'none', boxSizing: 'border-box', resize: 'vertical' }} />
            ) : (
              <div ref={editorRef} contentEditable suppressContentEditableWarning
                style={{ minHeight: '300px', padding: '16px', fontSize: '14px', lineHeight: '1.7', color: '#1a1a1a', background: '#fff', border: '1px solid #ddd', borderRadius: '0 0 8px 8px', outline: 'none', boxSizing: 'border-box' }}
                onFocus={(e) => { if (!e.currentTarget.textContent?.trim()) e.currentTarget.innerHTML = '<p>Cher(e) ami(e) du jazz,</p><p><br></p><p>Musicalement,<br>L\'equipe Jazz en Tech</p>' }}
              />
            )}
          </div>
          ...
          <details className="mb-5">
            ...
            <div className="rounded-lg p-4 overflow-auto" style={{ background: '#e5e2dc', maxHeight: '500px' }}>
          ...
          {result && (
            <div className="mb-4 rounded-lg p-3" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
              <p style={{ color: '#166534', fontSize: '13px', margin: 0 }}>
                {result.sent} email(s) envoye(s){result.errors > 0 ? `, ${result.errors} erreur(s)` : ''}
              </p>
            </div>
          )}

          <button onClick={handleSend} disabled={loading}
            style={{ background: '#722f37', color: '#fff', border: 'none', borderRadius: '10px', padding: '14px 28px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', opacity: loading ? 0.6 : 1 }}>
