...
async function sendWithBrevo(email: string, subject: string, htmlContent: string): Promise<boolean> {
...
export async function POST(req: NextRequest) {
  ...
  for (const email of recipients) {
    const brevoOk = await sendWithBrevo(email, subject, emailHtml)
    if (brevoOk) { results.push({ email, status: 'sent', via: 'brevo' }); continue }

    const resendOk = await sendWithResend(email, subject, emailHtml)
    if (resendOk) { results.push({ email, status: 'sent', via: 'resend' }); continue }

    results.push({ email, status: 'error', error: 'Échec envoi' })
  }
